...
const Exchange = artifacts.require("./ExchangeOrdered.sol");
...
async function contractStateAsserts(expOrder) {
    ...
    assert.equal(order[2].toString(), expPrice.toString(), "price should be set in contract's order array");
    assert.equal(order[3].toString(), expOrder.amount, "amount should be set in contract's order array");
}
